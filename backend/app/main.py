...
FRONTEND_ASSETS = BACKEND_DIR.parent.parent / "frontend" / "src" / "assets"
...
def find_image(image_id: int) -> bytes | None:
    """Find a preloaded crack image by ID, checking backend/images and frontend assets."""
...
async def detect_preloaded(image_id: int, conf: float = 0.15):
    """Run YOLO inference on a preloaded crack image (crack_1 … crack_6)."""
    image_bytes = find_image(image_id)
    if image_bytes is None:
        raise HTTPException(
            status_code=404,
            detail=f"预置图片 crack_{image_id} 未找到。请将图片放入 backend/app/images/ 或 frontend/src/assets/ 目录",
        )
